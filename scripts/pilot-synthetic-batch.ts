...
function replayPersona(persona: SyntheticPersona) {
  ...
  const { result, hiddenScores } = calculatePilotEraPrint(answers);
  ...
  return {
    ...
    hiddenScores: Object.fromEntries(HIDDEN_ERA_CODES.map((code) => [code, hiddenScores[code].score])),
...
async function main() {
  ...
  const knownQuestionIds = new Set(PILOT_QUESTIONS.map(({ id }) => id));
  for (const persona of dataset.personas) {
    for (const questionId of knownQuestionIds) {
      if (!(questionId in persona.answers)) throw new Error(`Persona ${persona.id} is missing ${questionId} from its complete answer map.`);
    }
  }
...
main().catch((error: unknown) => {
  console.error(error instanceof Error ? error.message : "Unable to run synthetic pilot batch.");
  process.exitCode = 1;
});
